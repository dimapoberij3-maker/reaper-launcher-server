...
# ==================== НАСТРОЙКИ СЕРВЕРА ====================
BOT_TOKEN = "8963416771:AAHIlA7tiWh6e6fjNLqqkwBj_o2x8n8oBK0"  # <--- Сюда токен от @BotFather
CURRENT_VERSION = "1.0"
...
def init_db():
    try:
        ...
        # Сносим старую таблицу при деплое, чтобы структура точно обновилась
...
async def admin_buttons_handler(message):
    # Команда регистрации для админа вшита прямо сюда
...
# --- ОБРАБОТКА ТЕКСТОВЫХ АДМИН-КОМАНД (ФИКС ВЫБОРКИ ИЗ КОРТЕЖА ПО ИНДЕКСУ [0]) ---
...
async def cmd_subscribe(message):
    ...
    try:
        ...
        current_expires = row[0]  # <--- ФИКС: Извлекаем дату из кортежа psycopg2
        now = datetime.now()
